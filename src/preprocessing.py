...
Handles:
  - Human NaN-gap interpolation + zero-mean
...
def preprocess_human(df_human: pd.DataFrame, fs: float = 1.0) -> pd.DataFrame:
    """
    Fill NaN gaps by linear interpolation, compute zero-mean signal.

    Adds columns to *df_human* **in place**:
      - temp_interp : interpolated temperature (°C)
      - temp_zm      : zero-mean version
    ...
    temp_zm = temp_interp - np.mean(temp_interp)

    duration_h = len(temp_zm) / fs / 3600
    print(f"\nAfter interpolation:")
    print(f"Signal length: {len(temp_zm)} samples")
    print(f"Duration: {duration_h:.1f} hours ({duration_h / 24:.1f} days)")
    print(f"Sampling rate: {fs} Hz")
    print(f"Mean removed: {np.mean(temp_interp):.3f} °C")
    print(f"Remaining NaNs: {np.isnan(temp_zm).sum()}")
    print(
        f"Signal range after zero-mean: [{temp_zm.min():.3f}, {temp_zm.max():.3f}] °C")
